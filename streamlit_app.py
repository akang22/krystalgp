            ...
            try:
                ...
                if not pd.isna(expected_ebitda) and expected_ebitda != "":
                    ...
                    if fuzzy_match_ebitda(
                        opportunity.ebitda_millions, float(expected_ebitda), tolerance=0.5
                    ):
                        correct_ebitda += 1
                ...
                if not pd.isna(expected_company) and expected_company != "":
                    ...
                    if (
                        ...
                        and expected_company.lower() in opportunity.company_name.lower()
                    ):
                        correct_company += 1
...
def page_side_by_side():
    ...
    col1.metric(
        "EBITDA",
        f"${gt_row['ebitda_millions']}M" if not pd.isna(gt_row["ebitda_millions"]) else "N/A",
    )
...
def page_batch_processing():
    ...
    selected_parsers = st.multiselect(
        "Select Parsers", parser_names, default=parser_names[:1] if parser_names else []
    )
    ...
    num_emails = st.slider(
        "Number of emails to process", 1, len(email_files), min(10, len(email_files))
    )
